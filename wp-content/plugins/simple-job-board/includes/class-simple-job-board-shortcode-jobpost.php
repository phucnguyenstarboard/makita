...
class Simple_Job_Board_Shortcode_Jobpost {
    ...
    public function jobpost_shortcode($atts, $content) {
        ...
        $args = apply_filters(
            ...
            'jobpost_category' => (!empty($_GET['selected_category']) && -1 != $_GET['selected_category'] ) ? esc_attr($_GET['selected_category']) : esc_attr($shortcode_args['category']),
            'jobpost_job_type' => (!empty($_GET['selected_jobtype']) && -1 != $_GET['selected_jobtype'] ) ? esc_attr($_GET['selected_jobtype']) : esc_attr($shortcode_args['type']),
            'jobpost_location' => (!empty($_GET['selected_location']) && -1 != $_GET['selected_location'] ) ? esc_attr($_GET['selected_location']) : esc_attr($shortcode_args['location']),
